//
// Created by Xu Peng on 2019/8/24.
//
...
int SophosClient::Setup()
{
    ...
    fread(this->Ks, sizeof(unsigned char), 16, f_rand);
    fread(this->prf_key, sizeof(unsigned char), 16, f_rand);
...
int SophosServer::save(unsigned char *label, unsigned char *IV, unsigned char *ciphertext)
{
    ...
    memcpy(buf, IV, 16);
    memcpy(buf + 16, ciphertext, 80);
    _value.assign((char*)buf, 96);
...
int SophosServer::search(TdpPK *pk, unsigned char *kw,  unsigned char *st, unsigned int counter,
